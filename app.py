...
with st.sidebar:
    ...
    if map_type.startswith("4."):
        sub_option = st.multiselect(
            ...
            default=list(POI_CATEGORIES.keys()),   # по умолчанию — всё
            help="По умолчанию выбраны все категории. Пустой выбор или "
                 "Select all = все подразделы. Цвет гекса — суммарное число "
                 "объектов, точки окрашены по категориям")
...
if _circ_center:
    # ОДНА кнопка на фичу: пока сводка не готова для текущего центра и
    # разрешений — «Сформировать», после расчёта она заменяется «Скачать».
    # Всегда включает комплект для res 8, плюс выбранный res, если он не 8.
    _res_list = list(dict.fromkeys([8, res_eff]))
    _exp = st.session_state.get("allmaps_export")
    _fresh = (_exp and _exp.get("center") == _circ_center
              and _exp.get("res_list") == _res_list)
    if not _fresh:
        if st.button("📊 Сформировать сводку по всем картам (Excel)",
                     help="Считает показатели всех 7 карт (трафик — пеший и "
                          ...
                          "текущего центра кругов и радиусов 1/2/5 км. Всегда "
                          "включает комплект для res 8; если выбрано другое "
                          "разрешение — добавит второй комплект листов. "
                          "Первый запуск 10–30 сек, дальше — из кэша."):
                    ...
                    try:
                        with pd.ExcelWriter(_buf, engine="openpyxl") as _w:
                            if len(_res_list) == 1:
                                ...
                                _par_df.to_excel(_w, sheet_name="Параметры",
                                                 index=False)
                                _sum_df.to_excel(_w, sheet_name="Сводка",
                                                 index=False)
                            else:
                                for _r in _res_list:
                                    _sum_df, _par_df = _results[_r]
                                    _par_df.to_excel(
                                        _w, sheet_name=f"Параметры res {_r}",
                                        index=False)
                                    _sum_df.to_excel(
                                        _w, sheet_name=f"Сводка res {_r}",
                                        index=False)
                        _mime = ("application/vnd.openxmlformats-"
                        ...
                        st.warning("⚠️ openpyxl не установлен — отдаю CSV. "
                                   "Для xlsx добавьте openpyxl>=3.1 в "
                                   "requirements.txt")
...
_src = None
if map_type.startswith("1."):
    _src = ("Kontur (2023)" if kontur_df is not None
            else "OSM: суррогат")
...
render_map(grid, series, unit, geo, map_type, marker=marker,
           ...
           # ══ ГАММА РАСКРАСКИ ШКАЛЫ (0.75) ══════════════════════════════
           # Показатель нелинейности раскраски гексов: цвет считается как
           # t = (v / vmax) ** gamma, где v — значение гекса, vmax — максимум
           # по городу, t — позиция на палитре COLORS (0 = синий, 1 = красный).
           #   < 1 (напр. 0.4): жёлто-оранжевые начинаются при меньших
           #     значениях, но разница между близкими ВЫСОКИМИ значениями
           #     стирается (у Kontur Population масса гексов в верхней
           #     половине диапазона — там нужна гамма ближе к 1 или выше).
           #   > 1: верх диапазона растянут, разница высоких значений
           #     хорошо различима, но низ дольше остаётся сине-зелёным.
